      ...
      <Modal open={state} onClose={handleConfirm}>
        <Box sx={{backgroundColor: "#fff"}}>
          ...
          <div className={styles.form}>
                      ...
                      infosAr ?
                        infosAr.map((row, index) => (
                          <TableRow sx={{"td:first-child": {borderLeftColor: "transparent"}}} key={index}>
                            ...
                            <TableCell><DoDisturbOnIcon onClick={() => remover(index)}
                                                        sx={{margin: "auto 0", ":hover": {color: "#666"}}}/></TableCell>
                          </TableRow>
                        )) : <></>
                    }
                  </TableBody>
            ...
            <Box sx={{display:"flex"}}>
                <Typography sx={{margin:"auto 0"}}>Qtd. Total: {qtd}</Typography>
                <Button sx={{margin:'1em'}} variant={"contained"} onClick={handleConfirm}>Fechar</Button>
                <Button sx={{margin:'1em'}} variant={"contained"} onClick={handleButtonClick} >Salvar</Button>
            </Box>
          </div>
        </Box>
      </Modal>
